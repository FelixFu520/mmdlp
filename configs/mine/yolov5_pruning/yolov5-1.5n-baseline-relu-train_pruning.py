...
widen_factor = 0.375    # The scaling factor that controls the width of the network structure
...
# ----------------- dataset变量
data_root = '/home/users/fa.fu/work/datasets/seed_dataset/'  # Root path of data
train_ann_file = 'seed_coco_20.json'    # Path of train annotation file
...
train_batch_size_per_gpu = 64   # Batch size of a single GPU during training
...
save_checkpoint_intervals = 100  # Save model checkpoint and validation intervals
default_scope = 'mmyolo'
log_level = 'INFO'
load_from = "/home/users/fa.fu/work/epoch_300_1.5n.pth"
resume = True
...
model = dict(
    type='mmdlp.YOLODetectorFix',
    data_preprocessor=dict(
        type='mmdlp.DetDataPreprocessorFix',
        mean=[0., 0., 0.],
        std=[255., 255., 255.],
        bgr_to_rgb=True),
    backbone=dict(
    ...
    neck=dict(
        ...
        act_cfg=dict(type='ReLU', inplace=True)),
    bbox_head=dict(
        type='mmdlp.YOLOv5HeadFix',
        head_module=dict(
            type='mmdlp.YOLOv5HeadModuleFix',
...
default_hooks = dict(
    ...
    checkpoint=dict(
        ...
        rule=None,
        max_keep_ckpts=-1),
    pruning_channels=dict(type="mmdlp.AfterTrainPruning", pruning_ratio=0.2, ignore_layers_name=["data_preprocessor", 'head'])
    )
custom_hooks = [
    dict(
        type='EMAHook',
