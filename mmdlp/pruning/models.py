...
from mmdlp.registry import MODELS, TASK_UTILS
...
class YOLODetectorFix(SingleStageDetector):
    ...
    def forward(self,
        ...
        elif mode == 'deploy_x_series':
            return self.deploy_x_series(inputs)
        else:
            raise RuntimeError(f'Invalid mode "{mode}". '
                               'Only supports loss, predict and tensor mode')

    def deploy_x_series(self, inputs):
        # data preprocesser
        # inputs = self.data_preprocessor.deploy(inputs)  # X系列芯片可以在外面处理
        # backbone
        x = self.extract_feat(inputs)
        # use bbox decoder
        assert hasattr(self.bbox_head, 'deploy_x_series')
        results = self.bbox_head.deploy_x_series(x)
        return results


@MODELS.register_module()
class DetDataPreprocessorFix(ImgDataPreprocessor):
    ...
    def __init__(self,
        ...
        self.register_buffer('mean_deploy',
                             torch.tensor(mean).view(1, -1, 1, 1), False)
        self.register_buffer('std_deploy',
    ...
    def deploy(self, batch_inputs: Tensor) -> Tensor:
        """For Export ONNX.
        batch_inputs -> (b, c, 1278, 1278)
            :resize(img_scale_factor=0.5) -> (b, c, 639, 639)
            :pad -> (b, c, 640, 640)

        """
        assert isinstance(batch_inputs, torch.Tensor), 'input data must be torch tensor type.'
        assert batch_inputs.ndim == 4, 'input data must be in [B, C, H, W] format.'
        # input must be float.
        if batch_inputs.dtype != torch.float32:
            batch_inputs = batch_inputs.float()  # 转成浮点型，否则interpolate报错
        # resize with img_scale_factor
        if self.img_scale_factor != 1.0:
            batch_inputs = F.interpolate(batch_inputs, scale_factor=self.img_scale_factor, mode='bilinear')
        # normalization
        batch_inputs = (batch_inputs - self.mean_deploy) / self.std_deploy
        # resize to divisable size. if 639, pad to 640.
        # # TODO: think one think
        # b, c, h, w = batch_inputs.shape
        # h_new = int(np.ceil(h / 32) * 32)
        # w_new = int(np.ceil(w / 32) * 32)
        # # pad zero to right and bottom.
        # batch_inputs[:, :, h:h_new, w:w_new] = 0.
        return batch_inputs
...
class DummyONNXNMSop(torch.autograd.Function):
    ...
    def symbolic(g, boxes, scores, max_output_boxes_per_class, iou_threshold,
        ...
        return g.op(
            ...
            score_threshold,
            outputs=1)
def add_nms_for_onnx(boxes,
                     scores,
                     max_output_boxes_per_class=1000,
                     iou_threshold: Tensor=None,
                     score_threshold: Tensor=None,
                     pre_top_k=-1,
                     after_top_k=-1,
                     labels=None):
    ...
    # for add a node before nms, incase trt op fix iou and score should is_weigh
    ...
    # mmcv op
    # dets, selected_indices = soft_nms(boxes, scores, iou_threshold, min_score=score_threshold)
    ...
    dets = torch.cat([boxes, scores], dim=2)  # (b, m, 5)
    return torch.cat([dets, labels.unsqueeze(2).float()], dim=2)

@MODELS.register_module()
class YOLOv5HeadFix(BaseDenseHead):
    ...
    def deploy(self, x: Tuple[Tensor], score_threshold: Tensor, iou_threshold: Tensor, add_nms: bool) -> Tensor:
        ...
        out = self(x)
        if len(out) == 2:
            cls_scores, bbox_preds = out
            objectnesses = None
            with_objectnesses = False
        else:
            cls_scores, bbox_preds, objectnesses = out
            with_objectnesses = True
        ...
        flatten_cls_scores = torch.cat(flatten_cls_scores, dim=1).sigmoid()  # (b, m, classes)

        # unfixed flatten_stride size is important.
        # If the shape does not change, use the previous mlvl_priors
        if featmap_sizes != self.featmap_sizes:
            self.mlvl_priors = self.prior_generator.grid_priors(
                featmap_sizes,
                dtype=cls_scores[0].dtype,
                device=cls_scores[0].device)  # [(6400, 2), (1600, 2), (400, 2)]
            self.featmap_sizes = featmap_sizes  # [(80, 80), (40, 40), (20, 20)]
        flatten_priors = torch.cat(self.mlvl_priors)  # (8400, 2)
        _bbox_preds = []
        for i, flatten_bbox_pred in enumerate(flatten_bbox_preds):
            _bbox_preds.append(flatten_bbox_pred * self.featmap_strides[i])

        flatten_bbox_preds = torch.cat(_bbox_preds, dim=1)  # (b, m, 4)

        # decode
        flatten_decoded_bboxes = self.bbox_coder.deploy(
            flatten_priors[None], flatten_bbox_preds)  # (b, m, 4)
        ...
        if with_objectnesses:
            ...
            flatten_objectness, _ = torch.max(flatten_cls_scores, dim=2, keepdim=True)  # (b, m, 1)

        if add_nms:
            return add_nms_for_onnx(
                            flatten_decoded_bboxes,
                            flatten_cls_scores,
                            score_threshold=score_threshold,
                            iou_threshold=iou_threshold)
        else:
            return torch.concat([flatten_decoded_bboxes, flatten_objectness, flatten_cls_scores], dim=2)

    def deploy_x_series(self, x: Tuple[Tensor]) -> Tensor:
        assert hasattr(self.head_module, 'deploy_x_series'), f'head module does not has deploy_x_series method'
        return self.head_module.deploy_x_series(x)
...
@MODELS.register_module()
class YOLOv5HeadModuleFix(BaseModule):
    ...
    def deploy_x_series(self, x: Tuple[Tensor]) -> Tuple[List]:
        assert len(x) == self.num_levels
        outs = []
        for i in range(len(x)):
            pred_temp = self.convs_pred[i](x[i])
            pred_temp = pred_temp.sigmoid()
            # outs.append(pred_temp)
            outs.append(pred_temp.permute(0, 2, 3, 1))
        return outs
